...
function rowToSkill(row) {
  return { id: row.id, name: row.name, text: row.text, updated_at: row.updated_at };
}
...
export async function listSkills(db) {
  const { results } = await db.prepare("SELECT id, name, text, updated_at FROM skills ORDER BY name ASC").all();
  return results.map(rowToSkill);
}

export async function loadSkill(db, id) {
  const row = await db.prepare("SELECT id, name, text, updated_at FROM skills WHERE id = ?").bind(id).first();
  return row ? rowToSkill(row) : null;
}

export async function findSkillByName(db, name) {
  const row = await db
    .prepare("SELECT id, name, text, updated_at FROM skills WHERE name = ?")
...
export async function createSkill(db, { name, text }) {
  ...
  await db
    .prepare("INSERT INTO skills (id, name, text, updated_at) VALUES (?, ?, ?, ?)")
    .bind(id, normalized, typeof text === "string" ? text : "", updated_at)
    .run();
  return loadSkill(db, id);
}

export async function updateSkill(db, id, { name, text }) {
  const existing = await loadSkill(db, id);
  if (!existing) return null;
  const normalized = typeof name === "string" && name.trim() ? name.toLowerCase() : existing.name;
  if (!isValidSkillName(normalized)) {
    throw new Error("Skill name must start with a letter and contain only letters, digits, and hyphens.");
  }
  if (normalized !== existing.name) {
    const clash = await findSkillByName(db, normalized);
    if (clash && clash.id !== id) throw new Error(`A skill named "${normalized}" already exists.`);
  }
  const updated_at = new Date().toISOString();
  await db
    .prepare("UPDATE skills SET name = ?, text = ?, updated_at = ? WHERE id = ?")
    .bind(normalized, typeof text === "string" ? text : existing.text, updated_at, id)
    .run();
  return loadSkill(db, id);
}

export async function deleteSkill(db, id) {
...
// An unrecognized slash command is left as literal text rather than
// silently eaten — a typo should read as obviously wrong, not vanish.
