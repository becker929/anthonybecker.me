...
OUT = Path("out/knobmap_hw002_kickgroup_v2_thr040"); OUT.mkdir(parents=True, exist_ok=True)
ROWS = [  # v2: the three dynamics knobs, with Threshold HELD at 0.4 so the compressor works
    ("compressor_attack_thr040", 4, 4, "Attack"), ("compressor_release_thr040", 4, 5, "Release"), ("compressor_ratio_thr040", 4, 2, "Ratio"),
]
HOLD = (4, 1, "Threshold", 0.4)   # (device, param, name, value) held for every row, restored at the end
STEPS = 13
live = LiveMcp()
def run(code):
    r = live.execute(code)
        ...
        try:
            for v in values:
                ...
                side.write_text(json.dumps(dict(row=row, device=meta["device"], device_index=di, param=pname, param_index=pi,
                    ...
                    note="Row is about the kick group bus as heard: kick + rumble, rumble's LFOTool on. Not a solo kick. Threshold HELD at 0.4 for this row so the compressor is working; at the set threshold it is idle.",
