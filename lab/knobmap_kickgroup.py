...
OUT = Path("out/knobmap_hw002_kickgroup_v1"); OUT.mkdir(parents=True, exist_ok=True)
ROWS = [  # research agent's order, comment 5644604509
    ("compressor_threshold", 4, 1, "Threshold"), ("compressor_attack", 4, 4, "Attack"), ("compressor_release", 4, 5, "Release"),
    ("roar_drive", 1, 1, "Drive"), ("roar_tone_amt", 1, 2, "Tone Amt"), ("roar_blend", 1, 5, "Blend"),
    ("compressor_ratio", 4, 2, "Ratio"), ("compressor_knee", 4, 12, "Knee"),
    ("roar_shaper1_amt", 1, 11, "Shaper 1 Amt"), ("roar_shaper1_bias", 1, 12, "Shaper 1 Bias"),
]
STEPS = 13
live = LiveMcp()
def run(code):
        ...
        try:
            for v in values:
                ...
                side.write_text(json.dumps(dict(row=row, device=meta["device"], device_index=di, param=pname, param_index=pi,
                    ...
                    bus=bus, note="Row is about the kick group bus as heard: kick + rumble, rumble's LFOTool on. Not a solo kick.",
