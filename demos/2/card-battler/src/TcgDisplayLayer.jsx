...
/* ── decks ───────────────────────────────────────────────────────────────
   Fixed order, no shuffle: the demo is meant to be reproducible. `power` is
   the only stat combat reads. `magic` is display payload — it changes what
   the board *looks* like when the card lands and strikes, never what happens. */
...
export default function TcgDisplayLayer() {
  ...
  const gate =
    scene.phase === "setup" ? { label: "Begin", act: advance, line: "Both sides draw three." }
      ...
      >
        ...
        {gate && (
          <div style={S.gate}>
            ...
            <button onClick={gate.act} style={S.gateButton}>
              {gate.label}
            </button>
          </div>
        )}
...
const S = {
  ...
  face: (card, mine, { perishing, perishMs = 720, spent, entrance, held, departing }) => {
    ...
    return {
      ...
      boxShadow: magic
        ? `0 2px 5px rgba(0,0,0,.35), inset 0 0 14px rgba(126,205,255,.28)`
        : held
